...
if [[ "${DOCKER_VERSION}" != 'null' ]] && [[ "${SKIP_DOCKER:-}" != '1' ]]; then
    ...
    docker run -u root -v "$PWD:$PWD" ghcr.io/ansible/community-ansible-dev-tools:latest \
        bash -c "whoami && id && chown -R 1000:1000 $PWD && ls -lh $PWD && [ -w $PWD ] && echo 'Mounts working' || { echo 'Mounts not working. You might need to either disable or make selinux permissive.'; exit 1; }"
fi
...
if [[ "${PODMAN_VERSION}" != 'null' ]] && [[ "${SKIP_PODMAN:-}" != '1' ]]; then
    ...
    podman run -u root -v "$PWD:$PWD" ghcr.io/ansible/community-ansible-dev-tools:latest \
        bash -c "whoami && id && chown -R 1000:1000 $PWD \
        && ls -lh $PWD && [ -w $PWD ] && echo 'Mounts working' || { echo 'Mounts not working. You might need to either disable or make selinux permissive.'; exit 1; }"
fi
